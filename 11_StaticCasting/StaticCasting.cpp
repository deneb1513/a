...
int main()
{
	...
	//기본 캐스팅 방법
	//강제로 CBase* 포인터에 메모리를 CChild1 클래스 메모리의 틀에 맞춰 읽어온다.
	CChild1* pChild1 = (CChild1*)pBase;
	pChild1->printB();

	//아래와 같이 포인트를 잘못 가르키는 실수를 할 경우 묹제가 발생할 수 있다.
	//강제로 int* 포인터에 메모리를 CChild*에 맞추면
	//서로 맞지 않아 문제가 차후에 발생할 수 있는데 문제 없이 컴파일이 된다.
	//이로 인해서 나중에 문제 발생시 어디서 문제가 발생했는는지 찾기가 어렵다.

	int* a = new int(0);
	pChild1 = (CChild1*)a;

	//Static Cast: 정적 캐스팅(컴파일 단계에서 변환)
	//강제 int* 포인터에 메모리를 CChild*에 맞추려고 하면 컴파일 불가
	//pChild1 = Static_cast<CChild1*>(a); <<-잘못된 예시
	//에러 출력:"잘못된 형식 변환입니다."
	...
	//자식이 달라도 확인하지 않기 때문에 에러x, 컴파일 문제x, 정상적으로 빌드됨
	//어디서 문제가 발생하였는지 찾기 어려움
