...
{
 /*정적 메모리 할당
 * 정적으로 메모리를 할당하면 컴파일 시 Stack영역에 할당되며, 함수를 빠져나가면 소멸된다.
 * 프로그램 컴파일 시 Stack에 얼마만큼의 크기로 할당을 해야하는지 결정되기 때문에, 컴파일 이후 크기변경 불가능
 ...
 */
	...
	//int자료형, 포인터 pArr선언 new연산자 사용: 동적 할당 배열.각 배열에 순서대로 중괄호의 값 대입
	for (int i = 0; i < 10; i++)//int i=0 으로 선언, i가 10보다 작을때까지만 반복, i값 증가
	{
		printf("%d", pArr[i]);//i값에 대응하는 배열 값을 출력
	}
	delete[] pArr;//delete[]로 배열임을 명시
	...
	a = b;//얕은 복사 : a의 주소값에 b의 주소값을 넣는다. b가 가리키고 있는걸 a도 가리키게 한다.
